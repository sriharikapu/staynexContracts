...
contract TradeableERC721 is ERC721Full, Ownable {
  ...
  function burn(uint256 _tokenidentiy) public {
    ...
    BookingManager RED34;

  function claimOwnershipRED(uint _tokenid) public {
    require(ownerOf(_tokenid) == msg.sender, "INVALID_OWNER");

    uint timestamp = block.timestamp;

    if (timestamp > RedeemTimeLimit && timestamp < TS23) {
        require(RED22._tokenid == _tokenid);
        RED22._tokenOwner = msg.sender;
        RED22._isclaimed = true;
    } else if (timestamp > TS23 && timestamp < TS24) {
        require(RED23._tokenid == _tokenid);
        RED23._tokenOwner = msg.sender;
        RED23._isclaimed = true;
    } else if (timestamp > TS24 && timestamp < TS25) {
        require(RED24._tokenid == _tokenid);
        RED24._tokenOwner = msg.sender;
        RED24._isclaimed = true;
    } else if (timestamp > TS25 && timestamp < TS26) {
        require(RED25._tokenid == _tokenid);
        RED25._tokenOwner = msg.sender;
        RED25._isclaimed = true;
    } else if (timestamp > TS26 && timestamp < TS27) {
        require(RED26._tokenid == _tokenid);
        RED26._tokenOwner = msg.sender;
        RED26._isclaimed = true;
    } else if (timestamp > TS27 && timestamp < TS28) {
        require(RED27._tokenid == _tokenid);
        RED27._tokenOwner = msg.sender;
        RED27._isclaimed = true;
    } else if (timestamp > TS28 && timestamp < TS29) {
        require(RED28._tokenid == _tokenid);
        RED28._tokenOwner = msg.sender;
        RED28._isclaimed = true;
    } else if (timestamp > TS29 && timestamp < TS30) {
        require(RED29._tokenid == _tokenid);
        RED29._tokenOwner = msg.sender;
        RED29._isclaimed = true;
    } else if (timestamp > TS30 && timestamp < TS31) {
        require(RED30._tokenid == _tokenid);
        RED30._tokenOwner = msg.sender;
        RED30._isclaimed = true;
    } else if (timestamp > TS31 && timestamp < TS32) {
        require(RED31._tokenid == _tokenid);
        RED31._tokenOwner = msg.sender;
        RED31._isclaimed = true;
    } else if (timestamp > TS32 && timestamp < TS33) {
        require(RED32._tokenid == _tokenid);
        RED32._tokenOwner = msg.sender;
        RED32._isclaimed = true;
    } else if (timestamp > TS33 && timestamp < TS34) {
        require(RED33._tokenid == _tokenid);
        RED33._tokenOwner = msg.sender;
        RED33._isclaimed = true;
    }
  }

  function getIsClaimed(address userAddress) public view returns (bool) {
    uint timestamp = block.timestamp;

    if (timestamp > RedeemTimeLimit && timestamp < TS23) {
        return RED22._tokenOwner == userAddress && RED22._isclaimed;
    } else if (timestamp > TS23 && timestamp < TS24) {
        return RED23._tokenOwner == userAddress && RED23._isclaimed;
    } else if (timestamp > TS24 && timestamp < TS25) {
        return RED24._tokenOwner == userAddress && RED24._isclaimed;
    } else if (timestamp > TS25 && timestamp < TS26) {
        return RED25._tokenOwner == userAddress && RED25._isclaimed;
    } else if (timestamp > TS26 && timestamp < TS27) {
        return RED26._tokenOwner == userAddress && RED26._isclaimed;
    } else if (timestamp > TS27 && timestamp < TS28) {
        return RED27._tokenOwner == userAddress && RED27._isclaimed;
    } else if (timestamp > TS28 && timestamp < TS29) {
        return RED28._tokenOwner == userAddress && RED28._isclaimed;
    } else if (timestamp > TS29 && timestamp < TS30) {
        return RED29._tokenOwner == userAddress && RED29._isclaimed;
    } else if (timestamp > TS30 && timestamp < TS31) {
        return RED30._tokenOwner == userAddress && RED30._isclaimed;
    } else if (timestamp > TS31 && timestamp < TS32) {
        return RED31._tokenOwner == userAddress && RED31._isclaimed;
    } else if (timestamp > TS32 && timestamp < TS33) {
        return RED32._tokenOwner == userAddress && RED32._isclaimed;
    } else if (timestamp > TS33 && timestamp < TS34) {
        return RED33._tokenOwner == userAddress && RED33._isclaimed;
    }
    
    // If no matching range is found, return false (not claimed)
    return false;
  }

  function getBookingOwnerRED(uint _tokenId) public view returns (address) {
    uint timestamp = block.timestamp;

    if (timestamp > RedeemTimeLimit && timestamp < TS23) {
        require(RED22._tokenid == _tokenId, "INVALID_TOKEN");
        return RED22._tokenOwner;
    } else if (timestamp > TS23 && timestamp < TS24) {
        require(RED23._tokenid == _tokenId, "INVALID_TOKEN");
        return RED23._tokenOwner;
    } else if (timestamp > TS24 && timestamp < TS25) {
        require(RED24._tokenid == _tokenId, "INVALID_TOKEN");
        return RED24._tokenOwner;
    } else if (timestamp > TS25 && timestamp < TS26) {
        require(RED25._tokenid == _tokenId, "INVALID_TOKEN");
        return RED25._tokenOwner;
    } else if (timestamp > TS26 && timestamp < TS27) {
        require(RED26._tokenid == _tokenId, "INVALID_TOKEN");
        return RED26._tokenOwner;
    } else if (timestamp > TS27 && timestamp < TS28) {
        require(RED27._tokenid == _tokenId, "INVALID_TOKEN");
        return RED27._tokenOwner;
    } else if (timestamp > TS28 && timestamp < TS29) {
        require(RED28._tokenid == _tokenId, "INVALID_TOKEN");
        return RED28._tokenOwner;
    } else if (timestamp > TS29 && timestamp < TS30) {
        require(RED29._tokenid == _tokenId, "INVALID_TOKEN");
        return RED29._tokenOwner;
    } else if (timestamp > TS30 && timestamp < TS31) {
        require(RED30._tokenid == _tokenId, "INVALID_TOKEN");
        return RED30._tokenOwner;
    } else if (timestamp > TS31 && timestamp < TS32) {
        require(RED31._tokenid == _tokenId, "INVALID_TOKEN");
        return RED31._tokenOwner;
    } else if (timestamp > TS32 && timestamp < TS33) {
        require(RED32._tokenid == _tokenId, "INVALID_TOKEN");
        return RED32._tokenOwner;
    } else if (timestamp > TS33 && timestamp < TS34) {
        require(RED33._tokenid == _tokenId, "INVALID_TOKEN");
  ...
  function getNightsRED(uint _tokenId) public view returns (uint) {
    uint timestamp = block.timestamp;
  ...
  function modifyNightsRED(address _tokenOwner, uint _nights, uint _flag) internal {
    uint timestamp = block.timestamp;

    if (timestamp > RedeemTimeLimit && timestamp < TS23) {
        require(RED22._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED23._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED24._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED25._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED26._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED27._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED28._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED29._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED30._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED31._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED32._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
        ...
        require(RED33._tokenOwner == _tokenOwner, "INVALID_TOKEN_OWNER");
  ...
  function RedeemR(uint256 _tokenidentity, address _customerAddress, uint256 _numberDays, uint _redeem) public {
    require(block.timestamp > RedeemTimeLimit); 
    require(block.timestamp < TS34);
    
    if (_redeem == 1) {
        require(ownerOf(_tokenidentity) == msg.sender, "Only NFT owner can redeem nights");
        uint256 nRED = getNightsRED(_tokenidentity) + _numberDays;
        require(nRED < MAX);
        ...
        modifyNightsRED(msg.sender, _numberDays, 1);
    } else if (_redeem == 0) {
        require(msg.sender == owner(), "Only Hotel Owner can revert the nights");
        require(ownerOf(_tokenidentity) == _customerAddress);
        uint256 nRED = getNightsRED(_tokenidentity);
        require(nRED > _numberDays);
        
        uint256 decimal = 10**18;
        uint256 IXI_PER_REFUND = SafeMath.mul(_numberDays, decimal);
        
        token.transfer(_customerAddress, IXI_PER_REFUND);
        modifyNightsRED(_customerAddress, _numberDays, 0);
    }
  }
